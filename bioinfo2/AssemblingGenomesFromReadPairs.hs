...
unbalancedOut inDeg outDeg = filter f $ nub $ Map.keys outDeg ++ Map.keys inDeg
...
unbalancedIn inDeg outDeg = filter f $ nub $ Map.keys outDeg ++ Map.keys inDeg
    where f x = case (Map.lookup x inDeg, Map.lookup x outDeg) of
                    (Just a, Just b) | a > b -> True
                    (Just _, Nothing) -> True
                    _ -> False
...
main = do
    ...
    let [k,d] = map read $ words $ ls !! 0 :: [Int]
    let kmers = tail ls
    let prefixes = map (intercalate "|" . map (take $ k-1) . splitOn "|") kmers
    let suffixes = map ( (: []) . intercalate "|" . map (drop 1) . splitOn "|") kmers
