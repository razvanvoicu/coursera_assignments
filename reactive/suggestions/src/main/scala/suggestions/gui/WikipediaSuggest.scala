...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.concatRecovered[String] { 
      _ => {
             val sel = suggestionList.selection.items.toList
             sel match {
                case List(x:String) => Observable.just(x)
