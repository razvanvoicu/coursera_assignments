...
// <<< Don't Change
    ...
    public static class TopTitlesMap extends Mapper<Text, Text, NullWritable, TextArrayWritable> {
        ...
        public void map(Text key, Text value, Context context) throws IOException, InterruptedException {
            collect.add(new Pair<IntWritable,Text>(new IntWritable(Integer.parseInt(value.toString())),new Text(key.toString())));
        }
        ...
        protected void cleanup(Context context) throws IOException, InterruptedException {
            ...
            for (int i = 0 ; i < pairs.length ; i++ ) {
                strings[i] = ((Pair<IntWritable,Text>)pairs[i]).first.toString() + " " + ((Pair<IntWritable,Text>)pairs[i]).second.toString();
            }
    ...
    public static class TopTitlesReduce extends Reducer<NullWritable, TextArrayWritable, Text, IntWritable> {
        ...
        public void reduce(NullWritable key, Iterable<TextArrayWritable> values, Context context) throws IOException, InterruptedException {
            for (TextArrayWritable ta : values)
                for (Writable t: ta.get()) {
                    String[] tokens = ((Text)t).toString().split(" ");
                    collect.add(new Pair(new IntWritable(Integer.parseInt(tokens[0])), new Text(tokens[1])));
                }
            Collections.sort(collect);
            Object[] pairs = collect.subList(collect.size()-N,collect.size()).toArray();
            for ( int i = pairs.length - 1; i >= 0 ; i-- )
                context.write(((Pair<IntWritable,Text>)pairs[i]).second,((Pair<IntWritable,Text>)pairs[i]).first);
        }
    }
}
