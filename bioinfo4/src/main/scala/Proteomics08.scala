...
object Proteomics08 {
  ...
  def size(spectrum: Array[Int], maxScore: Int, idx: Int, t: Int): Int = {
    (idx,t) match {
      case (0,0) ⇒ 1
      case (0,_) ⇒ 0
      case (idx,_) if idx < 0 ⇒ 0
      case (_,t) if t < 0 ⇒ 0
      case (idx,t) ⇒ masses.values.toList.map {
        v:Int ⇒ cache.get((idx-v,t-spectrum(idx))) match {
          case None ⇒
            val sz = size(spectrum, maxScore, idx-v, t-spectrum(idx))
            cache += (idx-v,t-spectrum(idx)) -> sz
            sz
          case Some(sz) ⇒ sz
        }
      }.sum
  ...
  def main(args: Array[String]) : Unit = {
    ...
    println
    val result = (threshold to maxScore).map(t ⇒ size(spectrum, maxScore, spectrum.size-1,t)).sum
    println(result)
  }
}
